 ...
 */
providerSchema.pre('save', function save(next) {
  const provider = this;
 ...
 */
providerSchema.methods.comparePassword = function comparePassword(candidatePassword, cb) {
 ...
 */
providerSchema.methods.gravatar = function gravatar(size) {
  if (!size) {
    size = 200;
  }
  if (!this.email) {
